...
for (let i = 0; i < countries[i].devices.length; i++) {
 
    function mapRadius(countries) {
        if (countries[i].devices === 0) {
            return 1;
        }
       
        return countries[i].devices * 2;
    }   
}
    // Create map legend 
    ...
    legend.onAdd = function() {
        ...
        let devices_country = [countries[i].Country];
        let labels = [];
        ...
        for (let i = 0; i < devices_country.length; i++) {
              labels.push('<ul style="background-color:' + chooseColor(devices_country[i] + 1) + '"> <span>' + devices_country[i] + (devices_country[i + 1] ? '&ndash;' + devices_country[i + 1] + '' : '+') + '</span></ul>');
            }
